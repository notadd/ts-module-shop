...
let GoodsService = class GoodsService {
    ...
    getGoodses(classifyId, pageNumber, pageSize) {
        return __awaiter(this, void 0, void 0, function* () {
            const classify = yield this.thirdClassifyRepository.findOne(classifyId);
            if (!classify) {
    ...
    getGoods(id) {
        return __awaiter(this, void 0, void 0, function* () {
            const goods = yield this.goodsRepository.findOne(id, { relations: ["type", "values"] });
            if (!goods) {
    ...
    createGoods(name, no, basePrice, discountPrice, description, classifyId, goodsTypeId, brandId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const type = yield this.goodsTypeRepository.findOne(goodsTypeId);
            if (!type) {
                throw new common_1.HttpException("指定id" + goodsTypeId + "商品类型不存在", 404);
            }
            let brand;
            if (brandId) {
                brand = yield this.brandRepository.findOne(brandId);
                if (!brand) {
    ...
    updateGoods(id, name, no, basePrice, discountPrice, description, classifyId, goodsTypeId, brandId) {
        return __awaiter(this, void 0, void 0, function* () {
            const goods = yield this.goodsRepository.findOne(id, { relations: ["classify", "type", "values", "brand"] });
            if (!goods) {
                throw new common_1.HttpException("指定id=" + id + "商品不存在", 404);
            }
            if (goods.recycle) {
            ...
            if (brandId && (brandId !== goods.brand.id)) {
                const brand = yield this.brandRepository.findOne(brandId);
                if (!brand) {
                    throw new common_1.HttpException("指定id" + brandId + "品牌不存在", 404);
                }
                goods.brand = brand;
    ...
    deleteGoods(id) {
        return __awaiter(this, void 0, void 0, function* () {
            const goods = yield this.goodsRepository.findOne(id);
            if (!goods) {
                throw new common_1.HttpException("指定id=" + id + "商品不存在", 404);
            }
            if (!goods.recycle) {
