...
let ClassifyService = class ClassifyService {
    ...
    getClassifes(parentId, level) {
        return __awaiter(this, void 0, void 0, function* () {
            if (level === 1) {
                ...
                let parent;
                if (parentId) {
                    parent = yield this.firstClassifyRepository.findOne(parentId);
                    if (!parent) {
                ...
                let parent;
                if (parentId) {
                    parent = yield this.secondClassifyRepository.findOne(parentId);
                    if (!parent) {
                        throw new common_1.HttpException("指定id=" + parentId + "上级分类不存在", 404);
                    }
    ...
    getClassify(id, level) {
        return __awaiter(this, void 0, void 0, function* () {
            let result;
            if (level === 1) {
                result = yield this.firstClassifyRepository.findOne(id);
            }
            else if (level === 2) {
                result = yield this.secondClassifyRepository.findOne(id);
            }
            else {
                result = yield this.thirdClassifyRepository.findOne(id);
            }
            return result;
        });
    }
    createClassify(name, description, level, parentId) {
        return __awaiter(this, void 0, void 0, function* () {
            if (level === 1) {
                ...
                const parent = yield this.firstClassifyRepository.findOne(parentId);
                if (!parent) {
                    throw new common_1.HttpException("指定id=" + parentId + "上级分类不存在", 404);
                }
                try {
                ...
                const parent = yield this.secondClassifyRepository.findOne(parentId);
                if (!parent) {
                    throw new common_1.HttpException("指定id=" + parentId + "上级分类不存在", 404);
                }
                try {
    ...
    updateClassify(id, name, description, level) {
        return __awaiter(this, void 0, void 0, function* () {
            if (level === 1) {
                const classify = yield this.firstClassifyRepository.findOne(id);
                if (!classify) {
                ...
                const classify = yield this.secondClassifyRepository.findOne(id);
                if (!classify) {
                ...
                const classify = yield this.thirdClassifyRepository.findOne(id);
                if (!classify) {
    ...
    deleteClassify(id, level) {
        return __awaiter(this, void 0, void 0, function* () {
            if (level === 1) {
                const classify = yield this.firstClassifyRepository.findOne(id, { relations: ["children"] });
                if (!classify) {
                    throw new common_1.HttpException("指定id=" + id + "一级分类不存在", 404);
                }
                ...
                const classify = yield this.secondClassifyRepository.findOne(id, { relations: ["children"] });
                if (!classify) {
                    throw new common_1.HttpException("指定id=" + id + "二级分类不存在", 404);
                }
                if (classify.children && classify.children.length > 0) {
                ...
                const classify = yield this.thirdClassifyRepository.findOne(id, { relations: ["goodses"] });
                if (!classify) {
                    throw new common_1.HttpException("指定id=" + id + "三级分类不存在", 404);
                }
