...
let OrderService = class OrderService {
    ...
    createOrder(userId, delivertNo, delivertTime, invoiceType, invoiceContent, invoiceTitle, customerMessage, deliveryId, userReceivingInformationId, items) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const delivery = yield this.deliveryRepository.findOne(deliveryId);
            if (!delivery) {
                throw new common_1.HttpException("指定id=" + deliveryId + "配送信息不存在", 404);
            }
            const userReceivingInformation = yield this.userReceivingInformationRepository.findOne(userReceivingInformationId);
            ...
            for (let i = 0; i < items.length; i++) {
                const sku = yield this.skuRepository.findOne(items[i].skuId);
                if (!sku) {
            ...
            try {
                for (let i = 0; i < items.length; i++) {
                    yield queryRunner.manager.save(orderItems[i]);
                    orderItems[i].sku.inventory -= items[i].count;
                    yield queryRunner.manager.save(orderItems[i].sku);
                }
                yield queryRunner.manager.save(this.orderRepository.create({ orderNo, userId, delivertNo, delivertTime: new Date(delivertTime), invoiceType, invoiceContent, invoiceTitle, customerMessage, delivery, userReceivingInformation, orderItems }));
