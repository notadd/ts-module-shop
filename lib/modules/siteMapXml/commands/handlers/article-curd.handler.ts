...
export class ArticleCurdHandler implements ICommandHandler<ArticleParamCommand>{
    ...
    async execute(command:ArticleParamCommand,resolver:(value) => void):Promise<any>{
        ...
        if(!command.article.getAllArticles){
            ...
            //分页获取全部文章
            result=await this.articleService.getArticleAll(command.article.limitNum,false,command.article.pages);
            console.log('command='+JSON.stringify(command));
        }
        //分页获取全部文章：可以选择是否隐藏
        if(command.article.getArticles.getArticleAll){
        ...
        //根据id获取单独页面
        if(command.article.getArticles.getArticleById){
            result=await this.articleService.getArticleById(command.article.getArticles.getArticleById)
        }
        //关键字搜索文章
        if(command.article.getArticles.keywordsSerach){
        ...
        //回收站获取文章
        if(command.article.getArticles.recycleFind){
            result=await this.articleService.recycleFind(command.article.limitNum,command.article.pages);
        }
        //回收站内根据分类id获取文章
        if(command.article.getArticles.reductionGetByClassifyId){
        ...
        if(command.article.getArticles.getArticleByClassifyId){
